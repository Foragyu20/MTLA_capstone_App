...
class TagalogDictionaryList extends StatefulWidget {
  const TagalogDictionaryList({Key? key}) : super(key: key);

  @override
  TagalogDictionaryListState createState() => TagalogDictionaryListState();
}

class TagalogDictionaryListState extends State<TagalogDictionaryList> {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        records = records.where((record) {
          return record['tagalog_word'].toLowerCase().contains(searchText.toLowerCase());
        }).toList();
      });
    } else {
      // Handle error
...
void handleWordTap(word,partofspeech,definition,example,translate1,translate2,translate3) {
  ...
  Widget build(BuildContext context) {
   ...
   drawer: const Homez(),
      appBar:  AppBar(
        backgroundColor: Colores.skin.withOpacity(1),
        title: const ListTile(title:Text('Filipino Dictionary'),) ,
      ),
      backgroundColor: Colores.skin.withOpacity(1),
      body: Container(
        ...
        child: Column(
          ...
          children: [
            Container(
              color:Colores.skin.withOpacity(1),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Container(
                    color: Colores.skin.withOpacity(0.3),
                    child: ListTile(
                      title:
                          SizedBox(
                            width: 140,
...
void speak(text) async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(''),
        backgroundColor: Colores.skin.withOpacity(0.3),
      ),
      backgroundColor: Colores.skin.withOpacity(1),
      body: 
      Container(
